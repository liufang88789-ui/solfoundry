...
def validate_solana_wallet(address: str) -> str:
    ...
    if not _BASE58_RE.match(address):
        raise ValueError(
            "Wallet must be a valid Solana base-58 address (32-44 alphanumeric characters, no 0/O/I/l)"
        )
    if address in KNOWN_PROGRAM_ADDRESSES:
        raise ValueError(
            f"Wallet '{address}' is a known program address and cannot receive payouts"
        )
    return address
...
class PayoutRecord(BaseModel):
    ...
    id: str = Field(
        ...
        description="Unique payout identifier (UUID)",
    )
    recipient: str = Field(
        ...,
        min_length=1,
        max_length=100,
        description="Recipient username or GitHub handle",
    )
    recipient_wallet: Optional[str] = Field(
        default=None, description="Recipient Solana wallet address"
    )
    amount: float = Field(
        ..., gt=0, description="Payout amount in the specified token (must be positive)"
    )
    token: str = Field(
        default="FNDRY",
        pattern=r"^(FNDRY|SOL)$",
        description="Token type: FNDRY or SOL",
    )
    bounty_id: Optional[str] = Field(
        default=None, description="Associated bounty UUID for double-pay prevention"
    )
    bounty_title: Optional[str] = Field(
        default=None, max_length=200, description="Human-readable bounty title"
    )
    tx_hash: Optional[str] = Field(
        default=None, description="On-chain Solana transaction signature"
    )
    status: PayoutStatus = Field(
        default=PayoutStatus.PENDING, description="Current lifecycle state"
    )
    solscan_url: Optional[str] = Field(
        default=None, description="Solscan explorer link for the transaction"
    )
    admin_approved_by: Optional[str] = Field(
        default=None, description="Admin who approved or rejected this payout"
    )
    retry_count: int = Field(
        default=0, ge=0, description="Number of transfer retry attempts made"
    )
    failure_reason: Optional[str] = Field(
        default=None, description="Error message if the payout failed"
    )
    ...
    def validate_tx_hash(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None and not _TX_HASH_RE.match(v):
            raise ValueError(
                "tx_hash must be a valid Solana transaction signature (64-88 base-58 characters)"
            )
        return v
...
class PayoutCreate(BaseModel):
    ...
    bounty_id: Optional[str] = Field(
        None,
        description="Associated bounty UUID (enforces one payout per bounty)",
    ...
    tx_hash: Optional[str] = Field(
        None,
        description="Solana transaction signature (pre-confirmed payout)",
        examples=["5fX..."],
    )
...
class PayoutResponse(BaseModel):
    ...
    bounty_title: Optional[str] = Field(
        default=None, description="Human-readable bounty title"
    )
    tx_hash: Optional[str] = Field(
    ...
    solscan_url: Optional[str] = Field(
        default=None, description="Solscan explorer link"
    )
    retry_count: int = Field(default=0, description="Number of transfer retry attempts")
    failure_reason: Optional[str] = Field(
        default=None, description="Error message if payout failed"
    )
...
class AdminApprovalRequest(BaseModel):
    ...
    admin_id: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Admin identifier performing the action",
    )
...
class WalletValidationRequest(BaseModel):
    ...
    wallet_address: str = Field(
        ..., min_length=1, max_length=50, description="Solana address to validate"
    )
...
class WalletValidationResponse(BaseModel):
    ...
    valid: bool = Field(
        ..., description="Whether the address is valid for receiving payouts"
    )
    is_program_address: bool = Field(
        default=False, description="True if the address is a known program address"
    )
...
class BuybackRecord(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: str(uuid.uuid4()),
        description="Unique buyback identifier",
    )
    ...
    price_per_fndry: float = Field(
        ..., gt=0, description="Effective price per FNDRY in SOL"
    )
    tx_hash: Optional[str] = Field(
        default=None, description="On-chain transaction signature"
    )
...
class TokenomicsResponse(BaseModel):
    ...
    token_ca: str = Field(
        default="C2TvY8E8B75EF2UP8cTpTp3EDUjTgjWmpaGnT74VBAGS",
        description="Token contract address on Solana",
    )
    total_supply: float = Field(
        default=1_000_000_000.0, description="Total token supply"
    )
    circulating_supply: float = Field(
        default=0.0, description="Tokens in circulation (total - treasury)"
    )
    treasury_holdings: float = Field(default=0.0, description="Tokens held in treasury")
    total_distributed: float = Field(
        default=0.0, description="Total tokens distributed to contributors"
    )
    total_buybacks: float = Field(
        default=0.0, description="Total FNDRY acquired via buybacks"
    )
    total_burned: float = Field(
        default=0.0, description="Total tokens permanently burned"
    )
    fee_revenue_sol: float = Field(
        default=0.0, description="Total SOL collected in fees / buyback spend"
    )
